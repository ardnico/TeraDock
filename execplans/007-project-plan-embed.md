# Embed PROJECT_PLAN.md into an ExecPlan
...
Capture the full contents of `PROJECT_PLAN.md` inside a single ExecPlan document so a novice can read one file and access the complete internal design and phased implementation plan without hunting for external references.
...
- No technical surprises; this is a documentation collation task.

## Decision Log

- Decision: Embed the full `PROJECT_PLAN.md` content under `Artifacts and Notes` as an indented block to preserve the single-fence ExecPlan format requirements.
  Rationale: ExecPlans cannot include nested code fences, so indentation preserves the original text without violating formatting rules.
...
`PROJECT_PLAN.md` defines the internal design, architecture, and phased implementation for TeraDock. The request is to ensure that plan is fully reproduced inside an ExecPlan, keeping with `.agent/PLANS.md` rules that plans must be self-contained for a novice reader.

## Plan of Work

Copy the complete contents of `PROJECT_PLAN.md` into this ExecPlan as an indented block, preserving headings and formatting. Do not add or remove content from the original plan, and do not introduce nested code fences.

## Concrete Steps

- Working directory: `/workspace/TeraDock`.
- Read `PROJECT_PLAN.md`.
- Paste the full contents into `Artifacts and Notes` below, indented by four spaces.

## Validation and Acceptance

- This ExecPlan contains the full `PROJECT_PLAN.md` text verbatim.
- The embedded text is readable and retains its original headings and structure.

## Idempotence and Recovery

Re-running this step is safe; overwrite the embedded block with the latest `PROJECT_PLAN.md` contents if the source changes.
